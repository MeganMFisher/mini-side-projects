...
app.get('/api/vehicle/', function(req, res){
  ...
  console.log(req.query)
  db.userFirstLetters(req.query.userFirstStart, function(err, response){
    if(!err) {
      res.send(response);
    }
  })
...
app.put('/api/vehicle/:vehicleId/user/:userId', function(req, res){
    db.changeOwnership([Number(req.params.vehicleId), Number(req.params.userId)], function(err, response){
        if(!err){
            res.send(response);
        }
    })
...
app.delete('/api/user/:userId/vehicle/:vehicleId', function(req, res){
    db.removeOwnership(Number(req.params.vehicleId), function(err, response){
        if(!err) {
            res.send(response);
        } else {
          console.log(err)
...
app.delete('/api/vehicle/:vehicleId', function(req, res){
    db.removeVehicle(Number(req.params.vehicleId), function(err, response){
        if(!err) {
            res.send(response);
